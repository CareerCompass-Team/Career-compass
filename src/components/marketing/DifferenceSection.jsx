...
export default function DifferenceSection() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-16">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-4 border-b text-xs font-bold uppercase tracking-wider" style={{ borderColor: 'var(--border-2)', color: 'var(--text-3)' }}>
          <div>Feature</div>
        ...
        <div className="divide-y" style={{ borderColor: 'var(--border-3)' }}>
          {comparisons.map((row, i) => (
            <div key={i} className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 text-xs items-center transition-colors hover:bg-white/5 px-2 rounded-xl">
              ...
              <div className="flex items-start gap-2" style={{ color: 'var(--text-4)' }}>
